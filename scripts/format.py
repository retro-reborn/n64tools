...
def format_file(file_path, style="file"):
    ...
    try:
        subprocess.run(
            ["clang-format", f"-style={style}", "-i", str(file_path)],
            check=True
        )
...
def main():
    parser = argparse.ArgumentParser(description="Format C and H files using clang-format")
    parser.add_argument("directory", nargs="?", default=".", help="Directory to search for files")
    parser.add_argument("--style", default="file", help="Formatting style (default: file)")
    parser.add_argument("--non-recursive", action="store_true", help="Don't search recursively")
