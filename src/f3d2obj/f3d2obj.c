...
static arg_config default_config = {{0}, 0, NULL, NULL, {0, 0, 0}, 1.0f, 1};
...
static texture tile = {0xFFFFFFFF, -1, -1, IMG_FORMAT_RGBA, -1};
...
static void load_vertices(unsigned char *data, unsigned int offset,
  ...
  for (i = 0; i < count; i++) {
    if (i + index < DIM(vertex_buffer)) {
      ...
      if (obj_vert_count + 1 > obj_vert_allocated) {
        ...
        obj_vertices =
            realloc(obj_vertices, obj_vert_allocated * sizeof(*obj_vertices));
      }
...
static void add_texture(texture const *const tex) {
  ...
  for (i = 0; i < texture_count; i++) {
    if (textures[i].address == tex->address)
      return;
  }
...
static void generate_material_file(arg_config *config, char *mtl_filename,
                                   char *texture_dir) {
...
#define TEXTURE_LUT 0x4CE0
        ...
        INFO("Decoding texture %06X->%06X (%d x %d) type %d\n", t->address,
             rom_addr, t->width, t->height, text_type);
        switch (text_type) {
        case 0:
          retval = decode_block0(&rom[rom_addr], length, img_raw);
          break;
        case 1:
          retval = decode_block1(&rom[rom_addr], length, img_raw);
          break;
        case 2:
          retval = decode_block2(&rom[rom_addr], length, img_raw);
          break;
        case 3:
          retval = decode_block3(&rom[rom_addr], length, img_raw);
          break;
        case 6:
          retval = decode_block6(&rom[rom_addr], length, img_raw);
          break;
        default:
...
static int print_f3d(FILE *fout, unsigned int *dl_addr, arg_config *config) {
  ...
  case F3D_VTX: {
    ...
    bank = data[4];
    seg_offset = seg_address & 0x00FFFFFF;
    INFO("%14s %u %u %08X (%02X %06X)\n", "F3D_VTX", count, index, seg_address,
         bank, seg_offset);
    if (seg_data[bank] == NULL) {
      ...
      load_vertices(seg_data[bank], seg_offset, index, count,
                    config->translate);
      for (i = 0; i < count; i++) {
  ...
  case G_SETFOGCOLOR:
    INFO("%14s %3d, %3d, %3d, %3d\n", "G_SETFOGCOLOR", data[4], data[5],
         data[6], data[7]);
    break;
  case G_SETENVCOLOR:
    INFO("%14s %3d, %3d, %3d, %3d\n", "G_SETENVCOLOR", data[4], data[5],
         data[6], data[7]);
    break;
  case G_SETCOMBINE: {
    struct {
