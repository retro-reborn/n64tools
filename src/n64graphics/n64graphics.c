...
rgba *raw2rgba(const uint8_t *raw, int width, int height, int depth) {
  ...
  if (depth == 16) {
    for (int i = 0; i < width * height; i++) {
      ...
      img[i].green = SCALE_5_8(((raw[i * 2] & 0x07) << 2) |
                               ((raw[i * 2 + 1] & 0xC0) >> 6));
...
ia *png2ia(const char *png_filename, int *width, int *height) {
  ...
  case 4: // red, green, blue, alpha
    ...
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        int idx = j * w + i;
        int sum = data[channels * idx] + data[channels * idx + 1] +
                  data[channels * idx + 2];
        img[idx].intensity =
            (sum + 1) / 3; // add 1 to round up where appropriate
        if (channels == 4) {
          img[idx].alpha = data[channels * idx + 3];
        } else {
          img[idx].alpha = 0xFF;
...
const char *n64graphics_get_read_version(void) { return "stb_image 2.19"; }
...
static const format_entry format_table[] = {
    {"rgba16", IMG_FORMAT_RGBA, 16}, {"rgba32", IMG_FORMAT_RGBA, 32},
    {"ia1", IMG_FORMAT_IA, 1},       {"ia4", IMG_FORMAT_IA, 4},
    {"ia8", IMG_FORMAT_IA, 8},       {"ia16", IMG_FORMAT_IA, 16},
    {"i4", IMG_FORMAT_I, 4},         {"i8", IMG_FORMAT_I, 8},
