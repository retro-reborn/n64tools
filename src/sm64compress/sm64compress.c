...
typedef struct {
  ...
  enum { BLOCK_LEVEL, BLOCK_MIO0, BLOCK_RAW } type;
} block;
...
static void print_usage(void) {
  ERROR("Usage: sm64compress [-a ALIGNMENT] [-c] [-d] [-f] [-g] [-v] FILE "
        "[OUT_FILE]\n"
        "\n"
        "sm64compress v" SM64COMPRESS_VERSION
        ...
        "File arguments:\n"
        " FILE         input ROM file\n"
        " OUT_FILE     output compressed ROM file (default: replaces input "
...
static void add_ref(block *blk, unsigned level_script, unsigned offset,
                    unsigned char type) {
...
static int walk_scripts(block *blocks, int block_count, unsigned char *buf,
                        unsigned int in_length, unsigned level_script,
                        unsigned script_end) {
  ...
  while (off < script_end) {
      ...
      if (buf[off + 1] == 0x10) {
        ...
        if ((block_off & 0xFF000000) == (block_end & 0xFF000000) &&
          ...
          if (idx < 0) {
            ...
            block_count++;
            // recurse
            block_count = walk_scripts(blocks, block_count, buf, in_length,
                                       block_off, block_end);
          }
    ...
    // could increment by buf[off+1] command length, but trying to be smart
    // might miss things
    off += 0x4;
  }
  return block_count;
...
static int find_sequence_bank(block *blocks, int block_count,
  ...
  for (int i = 0; i < count; i++) {
    ...
    cur = offset + read_u32_be(&buf[offset + 8 * i]) +
          read_u32_be(&buf[offset + 8 * i + 4]);
    if (cur > end) {
      end = cur;
    }
  }
...
static void fix_f3d(unsigned char *buf, int len) {
  static const unsigned char f3d_combine_old[] = {0xFC, 0x12, 0x7F, 0xFF,
                                                  0xFF, 0xFF, 0xF8, 0x38};
  static const unsigned char f3d_combine_new[] = {0xFC, 0x12, 0x18, 0x24,
                                                  0xFF, 0x33, 0xFF, 0xFF};
  for (int i = 0; i < len; i++) {
...
static int sm64_compress_mio0(const compress_config *config,
                              unsigned char *in_buf, unsigned int in_length,
...
#define SEGMENT2_ROM_END 0x81BB64
  ...
  block_count = walk_scripts(block_table, block_count, in_buf, in_length,
                             ENTRY_SCRIPT, ENTRY_SCRIPT + 0x30);
...
#define EXT_ROM_OFFSET 0x800000
        ...
        for (int r = 0; r < blk->ref_count; r++) {
          if (blk->refs[r].type == 0x17) {
            ...
            ERROR("Block %08X ref %X:%X type = %02X\n", blk->old,
                  blk->refs[r].level, blk->refs[r].offset, blk->refs[r].type);
          }
        }
      } else {
        src = &in_buf[blk->old];
      ...
      for (int r = 0; r < blk->ref_count; r++) {
        if (blk->refs[r].type == 0xFF) {
          ...
          INFO("Updating ASM @ %08X: %08X %08X %08X %08X\n", offset,
               read_u32_be(&out_buf[offset + 0]),
               read_u32_be(&out_buf[offset + 4]),
               read_u32_be(&out_buf[offset + 8]),
               read_u32_be(&out_buf[offset + 0xC]));
          // ADDIU sign extends which causes the summed high to be 1 less if low
          // MSb is set
          if (addr_low & 0x8000) {
            addr_high++;
          }
          ...
          INFO("Updating ASM @ %08X: %08X %08X\n", 0xD4784,
               read_u32_be(&out_buf[0xD4784 + 0]),
               read_u32_be(&out_buf[0xD4784 + 4]));
          ...
          INFO("Updating ASM @ %08X: %08X %08X %08X %08X %08X\n", 0x101BB0,
               read_u32_be(&out_buf[0x101BB0 + 0]),
               read_u32_be(&out_buf[0x101BB0 + 4]),
               read_u32_be(&out_buf[0x101BB0 + 8]),
               read_u32_be(&out_buf[0x101BB0 + 0xC]),
          ...
          int level_idx =
              find_block(block_table, block_count, blk->refs[r].level);
          if (level_idx < 0) {
            ...
            INFO("Updating @ %08X:%08X %02X 0C %02X %02X %08X-%08X to %02X 0C "
                 "00 %02X %08X-%08X\n",
                 level->old, level->new, out_buf[offset], out_buf[offset + 2],
                 out_buf[offset + 3], read_u32_be(&out_buf[offset + 4]),
                 read_u32_be(&out_buf[offset + 8]), blk->refs[r].type,
                 out_buf[offset + 3], blk->new, blk->new_end);
  ...
  // TODO: figure out what is going on with custom level scripts 17 and 10 in
  // RAM expansion
  // TODO: move allocation of memory pool to 0x80400000+ and revert audio code
  // to use it? detect audio patch and fix it
