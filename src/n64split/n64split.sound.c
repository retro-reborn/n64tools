...
void parse_music_sequences(FILE *out, unsigned char *data, split_section *sec,
                           arg_config *args, strbuf *makeheader) {
  typedef struct {
    unsigned int start;
    unsigned int length;
  } sequence;
  ...
  fprintf(out,
          ".hword %d, (music_sequence_table_end - music_sequence_table) / 8\n",
          seq_bank.revision);
  fprintf(out, "music_sequence_table:\n");
  for (i = 0; i < seq_bank.count; i++) {
    sprintf(seq_name, "seq_%02X", i);
    fprintf(out,
            ".word (%s - music_sequence_table_header), (%s_end - %s) # 0x%05X, "
            "0x%04X\n",
            seq_name, seq_name, seq_name, seq_bank.seq[i].start,
            seq_bank.seq[i].length);
  }
  ...
  for (i = 0; i < seq_bank.count; i++) {
    ...
    write_file(m64_file, &data[sec->start + seq_bank.seq[i].start],
               seq_bank.seq[i].length);
...
void parse_sound_banks(FILE *out, unsigned char *data, split_section *secCtl,
                       split_section *secTbl, arg_config *args,
                       strbuf *makeheader) {
  ...
  for (i = 0; i < sound_banks.bank_count; i++) {
    for (j = 0; j < sound_banks.banks[i].instrument_count; j++) {
      if (sound_banks.banks[i].sounds[j].wav_prev != NULL) {
        ...
        extract_raw_sound(sound_dir, sfx_file,
                          sound_banks.banks[i].sounds[j].wav_prev,
                          sound_banks.banks[i].sounds[j].key_base_prev,
                          sound_data.data[i], 16000);
        sound_count++;
      }
      if (sound_banks.banks[i].sounds[j].wav != NULL) {
        sprintf(sfx_file, "Bank%uSound%u", i, j);
        extract_raw_sound(
            sound_dir, sfx_file, sound_banks.banks[i].sounds[j].wav,
            sound_banks.banks[i].sounds[j].key_base, sound_data.data[i], 16000);
        sound_count++;
      }
