...
void write_geolayout(FILE *out, unsigned char *data, unsigned int start,
  ...
  while (a < end) {
    ...
    case 0x09: // 09 00 00 [AA]
      ...
      break;
    case 0x0A: // 0A [AA] [BB BB] [NN NN] [FF FF] {EE EE EE EE}: set camera
               // frustum
      fprintf(out, " %d, %d, %d", read_s16_be(&data[a + 2]),
              read_s16_be(&data[a + 4]), read_s16_be(&data[a + 6]));
      if (data[a + 1] > 0) {
        cmd_len += 4;
    ...
    case 0x0D: // 0D 00 00 00 [AA AA] [BB BB]: set render range
      fprintf(out, " %d, %d", read_s16_be(&data[a + 4]),
              read_s16_be(&data[a + 6]));
      break;
    case 0x0E: // 0E 00 [NN NN] [AA AA AA AA]: switch/case
      fprintf(out, " %d, geo_switch_case_%08X", read_s16_be(&data[a + 2]),
              read_u32_be(&data[a + 4]));
      break;
    case 0x0F: // 0F 00 [TT TT] [XX XX] [YY YY] [ZZ ZZ] [UU UU] [VV VV] [WW WW]
               // [AA AA AA AA]
      fprintf(out, " %d, %d, %d, %d, %d, %d, %d", read_s16_be(&data[a + 2]),
              read_s16_be(&data[a + 4]), read_s16_be(&data[a + 6]),
              read_s16_be(&data[a + 8]), read_s16_be(&data[a + 10]),
              read_s16_be(&data[a + 12]), read_s16_be(&data[a + 14]));
      disasm_label_lookup(state, read_u32_be(&data[a + 0x10]), label);
      fprintf(out, ", %s", label);
      break;
    case 0x10: // 10 [AA] [BB BB] [XX XX] [YY YY] [ZZ ZZ] [RX RX] [RY RY] [RZ
               // RZ] {SS SS SS SS}: translate & rotate
    {
      unsigned char params = data[a + 1];
      unsigned char field_type = (params & 0x70) >> 4;
      unsigned char layer = params & 0xF;
      switch (field_type) {
      case 0: // 10 [0L] 00 00 [TX TX] [TY TY] [TZ TZ] [RX RX] [RY RY] [RZ RZ]
              // {SS SS SS SS}: translate & rotate
      ...
      case 1: // 10 [1L] [TX TX] [TY TY] [TZ TZ] {SS SS SS SS}: translate
        fprintf(out, "geo_translate %d, %d, %d, %d", layer,
                read_s16_be(&data[a + 2]), read_s16_be(&data[a + 4]),
                read_s16_be(&data[a + 6]));
        cmd_len = 8;
        break;
    ...
    case 0x11: // 11 [P][L] [XX XX] [YY YY] [ZZ ZZ] {SS SS SS SS}: ? scene graph
               // node, optional DL
    case 0x12: // 12 [P][L] [XX XX] [YY YY] [ZZ ZZ] {SS SS SS SS}: ? scene graph
               // node, optional DL
    case 0x14: // 14 [P][L] [XX XX] [YY YY] [ZZ ZZ] {SS SS SS SS}: billboard
               // model
      fprintf(out, " 0x%02X, %d, %d, %d", data[a + 1] & 0xF,
              read_s16_be(&data[a + 2]), read_s16_be(&data[a + 4]),
              read_s16_be(&data[a + 6]));
      if (data[a + 1] & 0x80) {
        disasm_label_lookup(state, read_u32_be(&data[a + 8]), label);
      ...
      break;
    case 0x13: // 13 [LL] [XX XX] [YY YY] [ZZ ZZ] [AA AA AA AA]: scene graph
               // node with layer and translation
    ...
    case 0x15: // 15 [LL] 00 00 [AA AA AA AA]: load display list
      fprintf(out, " 0x%02X, seg%X_dl_%08X", data[a + 1], data[a + 4],
              read_u32_be(&data[a + 4]));
      break;
    case 0x16: // 16 00 00 [AA] 00 [BB] [CC CC]: start geo layout with shadow
      fprintf(out, " 0x%02X, 0x%02X, %d", data[a + 3], data[a + 5],
              read_s16_be(&data[a + 6]));
      break;
    ...
    case 0x1C: // 1C [PP] [XX XX] [YY YY] [ZZ ZZ] [AA AA AA AA]
      disasm_label_lookup(state, read_u32_be(&data[a + 8]), label);
      fprintf(out, " 0x%02X, %d, %d, %d, %s", data[a + 1],
              read_s16_be(&data[a + 2]), read_s16_be(&data[a + 4]),
              read_s16_be(&data[a + 6]), label);
      break;
...
void generate_geo_macros(arg_config *args) {
  ...
  fprintf(
      ...
      ".endm\n"
      "\n"
      "# 0x10: Create translation & rotation scene graph node with optional "
      "display list\n"
      "# Four different versions of 0x10\n"
      "#   cmd+0x01: u8 params\n"
      "#     0b1000_0000: if set, enable displayList field and drawingLayer\n"
      "#     0b0111_0000: fieldLayout (determines how rest of data is "
      "formatted\n"
      ...
      "#     0x0C: s16 xRotation\n"
      "#     0x0E: s16 xRotation\n"
      "#     0x10: [u32 displayList: if MSbit of params set, display list "
      "segmented address]\n"
      ".macro geo_translate_rotate layer, tx, ty, tz, rx, ry, rz, "
      "displayList=0\n"
      "    .byte 0x10\n"
      "    .if (\\displayList != 0)\n"
      ...
      "#     0x08: [u32 displayList: if MSbit of params set, display list "
      "segmented address]\n"
      ...
      "#     0x02: s16 yRotation\n"
      "#     0x04: [u32 displayList: if MSbit of params set, display list "
      "segmented address]\n"
      ...
      "#   0x04: s16 yTranslation\n"
      "#   0x06: s16 zTranslation\n"
      "#   0x08: [u32 displayList: if MSbit of params is set, display list "
      "segmented address]\n"
      ...
      ".set SHADOW_SQUARE_PERMANENT, 0x0A  # square shadow that never "
      "disappears\n"
      ".set SHADOW_SQUARE_SCALABLE,  0x0B  # square shadow, shrinks with "
      "distance\n"
      ".set SHADOW_SQUARE_TOGGLABLE, 0x0C  # square shadow, disappears with "
      "distance\n"
      ".set SHADOW_CIRCLE_PLAYER,    0x63  # player (Mario) shadow\n"
      ".set SHADOW_RECTANGLE_HARDCODED_OFFSET, 0x32 # offset of hard-coded "
      "shadows\n"
      ...
      "#   0x02: s16 background: background ID, or RGBA5551 color if "
      "backgroundFunc is null\n"
