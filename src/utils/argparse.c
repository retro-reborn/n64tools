...
static int convert_value(const char *value, arg_type type, void *dest, const char **enum_values, int enum_count) {
    if (value == NULL) {
        return -1;
    }
...
arg_parser* argparse_init(const char *prog_name, const char *prog_version, const char *prog_description) {
...
int argparse_add_flag(arg_parser *parser, char short_flag, const char *long_flag, 
                     arg_type type, const char *help, const char *meta, 
                     void *dest, bool required, const char **enum_values, int enum_count) {
    arg_def *new_flags;
    ...
    new_flags = realloc(parser->flags, (parser->flag_count + 1) * sizeof(arg_def));
    if (new_flags == NULL) {
        return -1;
    }
...
int argparse_add_positional(arg_parser *parser, const char *name, const char *help, 
                           arg_type type, void *dest, bool required) {
    ...
    new_pos_args = realloc(parser->pos_args, (parser->pos_arg_count + 1) * sizeof(pos_arg_def));
...
void argparse_print_help(arg_parser *parser, FILE *out) {
    ...
    for (i = 0; i < parser->flag_count; i++) {
        ...
        if (parser->flags[i].short_flag != '\0' && parser->flags[i].long_flag != NULL) {
            width += 2; /* ", " */
        }
    ...
    /* Print description */
    fprintf(out, "%s v%s: %s\n\n", parser->prog_name, 
            parser->prog_version, parser->prog_description);
    ...
    if (parser->flag_count > 0) {
        ...
        for (i = 0; i < parser->flag_count; i++) {
            ...
            /* Print enum values */
            if (parser->flags[i].type == ARG_TYPE_ENUM && parser->flags[i].enum_values != NULL) {
                fprintf(out, " (choices: ");
