...
void load_child_node(split_section *section, yaml_document_t *doc,
                     yaml_node_t *node) {
  char val[MAX_SIZE];
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (next_node && next_node->type == YAML_SCALAR_NODE) {
      ...
      case 3:
        tex->width = strtoul(val, NULL, 0);
        break;
      case 4:
        tex->height = strtoul(val, NULL, 0);
        break;
      case 5:
        if (tex->format != TYPE_TEX_CI) {
          ERROR("Error: expected 5 fields for non-CI texture\n");
        } else {
          tex->palette = strtoul(val, NULL, 0);
          break;
        }
        break;
      }
...
void load_behavior(split_section *beh, yaml_document_t *doc,
                   yaml_node_t *node) {
  char val[64];
  yaml_node_item_t *i_node;
  yaml_node_t *next_node;
  ...
  i_node = node->data.sequence.items.start;
  if (count != 2) {
    ERROR("Error: " SIZE_T_FORMAT
          " - expected 2 fields for behavior, got " SIZE_T_FORMAT "\n",
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (next_node && next_node->type == YAML_SCALAR_NODE) {
      ...
      switch (i) {
      case 0:
        beh->start = strtoul(val, NULL, 0);
        break;
      case 1:
        strcpy(beh->label, val);
        break;
      }
    } else {
...
void load_section_data(split_section *section, yaml_document_t *doc,
                       yaml_node_t *node) {
  yaml_node_item_t *i_node;
  yaml_node_t *next_node;
  size_t count =
...
void load_section(split_section *section, yaml_document_t *doc,
                  yaml_node_t *node) {
  char val[MAX_SIZE];
  yaml_node_item_t *i_node;
  yaml_node_t *next_node;
  ...
  if (count >= 3) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (next_node && next_node->type == YAML_SCALAR_NODE) {
        ...
        switch (i) {
        case 0:
          section->start = strtoul(val, NULL, 0);
          break;
        case 1:
          section->end = strtoul(val, NULL, 0);
          break;
...
int load_sections_sequence(rom_config *c, yaml_document_t *doc,
                           yaml_node_t *node) {
  yaml_node_t *next_node;
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      if (next_node) {
        if (next_node->type == YAML_SEQUENCE_NODE) {
          ...
          for (i_node_p = next_node->data.mapping.pairs.start;
               i_node_p < next_node->data.mapping.pairs.top; i_node_p++) {
            ...
            if (key_node && val_node && key_node->type == YAML_SEQUENCE_NODE &&
                val_node->type == YAML_SEQUENCE_NODE) {
          ...
          ERROR("Error: sections sequence contains non-sequence and "
                "non-mapping\n");
        }
      }
    }
    ...
    if (count >= 2 && count <= 3) {
      ...
      for (size_t i = 0; i < count; i++) {
        ...
        if (next_node && next_node->type == YAML_SCALAR_NODE) {
          ...
          switch (i) {
          case 0:
            lab->ram_addr = strtoul(val, NULL, 0);
            break;
          case 1:
            strcpy(lab->name, val);
            break;
...
int load_labels_sequence(rom_config *c, yaml_document_t *doc,
                         yaml_node_t *node) {
  yaml_node_t *next_node;
  int ret_val = -1;
  if (node->type == YAML_SEQUENCE_NODE) {
...
void parse_yaml_root(yaml_document_t *doc, yaml_node_t *node, rom_config *c) {
  ...
  default:
    ERROR("Error: " SIZE_T_FORMAT
          " only mapping node supported at root level\n",
          node->start_mark.line);
    break;
  }
}
...
void config_print(const rom_config *config) {
  ...
  for (i = 0; i < config->section_count; i++) {
    printf("0x%08X 0x%08X 0x%08X %d %s %d\n", s[i].start, s[i].end, s[i].vaddr,
           s[i].type, s[i].label, s[i].child_count);
  ...
  for (i = 0; i < config->section_count; i++) {
    ...
    if (isec->end > max_len) {
      ERROR("Error: section %d \"%s\" (%X-%X) past end of file (%X)\n", i,
            isec->label, isec->start, isec->end, max_len);
      ret_val = -3;
    }
    ...
    for (j = 0; j < i; j++) {
      ...
      if (!((isec->end <= jsec->start) || (isec->start >= jsec->end))) {
        ERROR("Error: section %d \"%s\" (%X-%X) overlaps %d \"%s\" (%X-%X)\n",
              i, isec->label, isec->start, isec->end, j, jsec->label,
              jsec->start, jsec->end);
        // ret_val = -1;
      }
    }
    ...
    for (j = i + 1; j < config->label_count; j++) {
      if (config->labels[i].ram_addr == config->labels[j].ram_addr) {
        ERROR("Error: duplicate label %X \"%s\" \"%s\"\n",
              config->labels[i].ram_addr, config->labels[i].name,
              config->labels[j].name);
        ret_val = -5;
      }
      if (0 == strcmp(config->labels[i].name, config->labels[j].name)) {
        ERROR("Error: duplicate label name \"%s\" %X %X\n",
              config->labels[i].name, config->labels[i].ram_addr,
              config->labels[j].ram_addr);
        ret_val = -5;
      }
    }
  }
