// A lot of this code is heavily influenced by and even directly copied from the
// work done by SubDrag and
//  Ice Mario on the N64 Sound Tool and VADPCM decoding/encoding. This wouldn't
//  be possible without their enormous contribution. Thanks guys!
...
static wave_table *read_wave_table(unsigned char *data,
                                   unsigned int wave_offset,
                                   unsigned int sound_bank_offset) {
  ...
  if (predictor_offset != 0) {
    ...
    unsigned int num_predictor =
        wav->predictor->order * wav->predictor->predictor_count * 8;
    wav->predictor->data = malloc(num_predictor * sizeof(unsigned));
    for (unsigned int k = 0; k < num_predictor; k++) {
      wav->predictor->data[k] =
          read_u16_be(&data[predictor_offset + 8 + k * 2]);
    }
  }
...
void sfx_initialize_key_table() {
  for (int x = 0; x < 0xFF; x++) {
    sfx_key_table[x] = (float)((60.0 - (float)x) / 12.0) *
                       (float)((60.0 - (float)x) / 12.0); // Squared
  }
}
...
static signed short
sfx_sign_extend(unsigned b, // number of bits representing the number in x
...
static void decode_8(unsigned char *in, signed short *out, int index,
                     signed short *pred1, signed short lastsmp[8]) {
  int i;
  signed short tmp[8];
  signed long total = 0;
...
static void decode_8_half(unsigned char *in, signed short *out, int index,
                          signed short *pred1, signed short lastsmp[8]) {
  int i;
  signed short tmp[8];
  signed long total = 0;
...
static unsigned long decode(unsigned char *in, signed short *out,
                            unsigned long len, predictor_data *book,
                            int decode8Only) {
...
int extract_raw_sound(char *sound_dir, char *wav_name, wave_table *wav,
                      float key_base, unsigned char *snd_data,
  ...
  /*if (!ignoreKeyBase)
  {
     ...
     {
        samplingRateFloat = samplingRateFloat /
  CN64AIFCAudio::keyTable[alBank->inst[instrument]->sounds[sound]->key.keybase];
     }
     else if (
     ...
     {
        samplingRateFloat = samplingRateFloat / (((*reinterpret_cast<float*>
  (&alBank->inst[instrument]->sounds[sound]->unknown3) - 0.0) < 0.00001) ? 1.0f
  : *reinterpret_cast<float*>
  (&alBank->inst[instrument]->sounds[sound]->unknown3));
     }
  }*/
  ...
  signed short *out_raw_data =
      malloc(wav->sound_length * 4 * sizeof(signed short));
  int n_samples =
      decode(sndData, out_raw_data, wav->sound_length, wav->predictor, 0);

  unsigned long chunk_size = 0x28 + (n_samples * 2) + 0x44 - 0x8;

  unsigned char *wav_data =
      malloc((chunk_size + 0x8 + 0x4) * sizeof(unsigned char));
  ...
  if (wav->loop->start != 0 || wav->loop->count != 0) {
    ...
    {
      wav_data[wavIndex + 0x14] =
          sfx_convert_ead_game_value_to_key_base(key_base);
    }
...
sound_data_header read_sound_data(unsigned char *data,
  ...
  if (sound_data.data_count > 0) {
    ...
    for (i = 0; i < sound_data.data_count; i++) {
      unsigned int sound_data_offset =
          read_u32_be(&data[data_offset + i * 8 + 4]) + data_offset;
      unsigned int sound_data_length =
          read_u32_be(&data[data_offset + i * 8 + 8]);
...
sound_bank_header read_sound_bank(unsigned char *data,
                                  unsigned int data_offset) {
  ...
  if (sound_banks.bank_count > 0) {
    sound_banks.banks =
        malloc(sound_banks.bank_count * sizeof(*sound_banks.banks));
    for (i = 0; i < sound_banks.bank_count; i++) {
      unsigned int sound_bank_offset =
          read_u32_be(&data[data_offset + i * 8 + 4]) + data_offset;
      // unsigned int length = read_u32_be(&data[secCtl->start+i*8+8]);

      sound_banks.banks[i].instrument_count =
          read_u32_be(&data[sound_bank_offset]);
      sound_banks.banks[i].percussion_count =
          read_u32_be(&data[sound_bank_offset + 4]);
      sound_banks.banks[i].unknown_1 =
          read_u32_be(&data[sound_bank_offset + 8]);
      sound_banks.banks[i].unknown_2 =
          read_u32_be(&data[sound_bank_offset + 12]);

      // sounds
      if (sound_banks.banks[i].instrument_count > 0) {
        sound_banks.banks[i].sounds =
            malloc(sound_banks.banks[i].instrument_count *
                   sizeof(*sound_banks.banks[i].sounds));
        for (j = 0; j < sound_banks.banks[i].instrument_count; j++) {
          unsigned int sound_offset =
              read_u32_be(&data[sound_bank_offset + 20 + j * 4]);

          if (sound_offset != 0) {
            sound_offset += sound_bank_offset + 16;

            sound_banks.banks[i].sounds[j].unknown =
                read_u32_be(&data[sound_offset]);

            // adrs
            unsigned int adrs_offset = read_u32_be(&data[sound_offset + 4]);
            if (adrs_offset != 0) {
              sound_banks.banks[i].sounds[j].adrs =
                  malloc(8 * sizeof(unsigned));
              for (k = 0; k < 8; k++) {
                sound_banks.banks[i].sounds[j].adrs[k] = read_u16_be(
                    &data[adrs_offset + sound_bank_offset + 16 + k * 2]);
              }
            }
            ...
            if (wav_prev_offset != 0) {
              sound_banks.banks[i].sounds[j].wav_prev = read_wave_table(
                  data, wav_prev_offset + sound_bank_offset + 16,
                  sound_bank_offset);
            } else {
              sound_banks.banks[i].sounds[j].wav_prev = NULL;
            }
            sound_banks.banks[i].sounds[j].key_base_prev =
                read_f32_be(&data[sound_offset + 12]);

            // wav
            unsigned int wav_offset = read_u32_be(&data[sound_offset + 16]);
            if (wav_offset != 0) {
              sound_banks.banks[i].sounds[j].wav = read_wave_table(
                  data, wav_offset + sound_bank_offset + 16, sound_bank_offset);
            } else {
              sound_banks.banks[i].sounds[j].wav = NULL;
            }
            sound_banks.banks[i].sounds[j].key_base =
                read_f32_be(&data[sound_offset + 20]);

            // wav_sec
            unsigned int wav_sec_offset = read_u32_be(&data[sound_offset + 24]);
            if (wav_sec_offset != 0) {
              sound_banks.banks[i].sounds[j].wav_sec =
                  read_wave_table(data, wav_sec_offset + sound_bank_offset + 16,
                                  sound_bank_offset);
            } else {
              sound_banks.banks[i].sounds[j].wav_sec = NULL;
            }
            sound_banks.banks[i].sounds[j].key_base_sec =
                read_f32_be(&data[sound_offset + 28]);
          } else {
            sound_banks.banks[i].sounds[j].wav_prev = NULL;
            sound_banks.banks[i].sounds[j].wav = NULL;
            sound_banks.banks[i].sounds[j].wav_sec = NULL;
      ...
      if (sound_banks.banks[i].percussion_count > 0) {
        unsigned int perc_table_offset =
            read_u32_be(&data[sound_bank_offset + 16]) + sound_bank_offset + 16;

        sound_banks.banks[i].percussions.items =
            malloc(sound_banks.banks[i].percussion_count * sizeof(percussion));
        for (j = 0; j < sound_banks.banks[i].percussion_count; j++) {
          unsigned int perc_offset =
              read_u32_be(&data[perc_table_offset + j * 4]);

          if (perc_offset != 0) {
            perc_offset += sound_bank_offset + 16;
            sound_banks.banks[i].percussions.items[j].unknown_1 =
                data[perc_offset];
            sound_banks.banks[i].percussions.items[j].pan =
                data[perc_offset + 1];
            sound_banks.banks[i].percussions.items[j].unknown_2 =
                read_u16_be(&data[perc_offset + 2]);

            // wav
            unsigned int wav_offset = read_u32_be(&data[perc_offset + 4]);
            if (wav_offset != 0) {
              sound_banks.banks[i].percussions.items[j].wav = read_wave_table(
                  data, wav_offset + sound_bank_offset + 16, sound_bank_offset);
            }
            sound_banks.banks[i].percussions.items[j].key_base =
                read_f32_be(&data[perc_offset + 8]);

            // adrs
            unsigned int adrs_offset = read_u32_be(&data[perc_offset + 12]);
            if (adrs_offset != 0) {
              sound_banks.banks[i].percussions.items[j].adrs =
                  malloc(8 * sizeof(unsigned));
              for (k = 0; k < 8; k++) {
                sound_banks.banks[i].percussions.items[j].adrs[k] = read_u16_be(
                    &data[adrs_offset + sound_bank_offset + 16 + k * 2]);
              }
            }
          }
