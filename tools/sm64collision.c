...
static void print_usage(void) {
  ERROR("Usage: sm64collision [-o OFFSET] [-n NAME] [-s SCALE] [-x X] [-y Y] "
        "[-z Z] [-v] FILE [OUT_FILE]\n"
        "\n"
        "sm64collision v" SM64COLLISION_VERSION
        ": Super Mario 64 collision decoder\n"
        "\n"
        "Optional arguments:\n"
        " -o OFFSET    offset to pull collision data from in bytes (default: "
        "0x%X)\n"
...
void decode_collision(unsigned char *data, collision *col) {
  ...
  while (processing) {
    ...
    for (i = 0; i < cur_tcount; i++) {
      col->tris[tcount + i].vidx[0] =
          read_u16_be(&data[offset + i * v_per_t * 2]);
      col->tris[tcount + i].vidx[1] =
          read_u16_be(&data[offset + i * v_per_t * 2 + 2]);
      col->tris[tcount + i].vidx[2] =
          read_u16_be(&data[offset + i * v_per_t * 2 + 4]);
    }
    tcount += cur_tcount;
...
static void generate_obj(char *filename, char *name, collision *col,
                         unsigned scale) {
  FILE *out;
  unsigned i;
  ...
  for (i = 0; i < col->tcount; i++) {
    fprintf(out, "f %d %d %d\n", col->tris[i].vidx[0] + 1,
            col->tris[i].vidx[1] + 1, col->tris[i].vidx[2] + 1);
  }
